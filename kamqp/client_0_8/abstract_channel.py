...
class AbstractChannel(object):
    """Superclass for both the :class:`Connection`, which is treated
    as channel 0, and other user-created :class:`Channel` objects.

    The subclasses must have a :attr:`_METHOD_MAP` class attribute,
    mapping between AMQP method signatures and Python methods.
    ...
    def dispatch_method(self, method_sig, args, content):
        if content and self.auto_decode and \
        ...
        try:
            ...
            raise KeyError("Unknown AMQP method (%s %s)" % method_sig)
